...
extern std::map<uint32_t,uint32_t> gc_sge_map;
extern std::mutex gc_sge_map_lock;
class DB {
 public:
  ...
  virtual bool Get_MemTable_interlnal(const ReadOptions& options, 
                    ColumnFamilyHandle* column_family, const Slice& key,
                     PinnableSlice* value) ;
  virtual bool Get_MemTable(const ReadOptions& options, const Slice& key,
                     std::string* value) {
    assert(value != nullptr);
    PinnableSlice pinnable_val(value);
    assert(!pinnable_val.IsPinned());
    auto s = Get_MemTable_interlnal(options, DefaultColumnFamily(), key, &pinnable_val);
    if (s && pinnable_val.IsPinned()) {
      value->assign(pinnable_val.data(), pinnable_val.size());
    }  // else value is already assigned
    return s;   
  }

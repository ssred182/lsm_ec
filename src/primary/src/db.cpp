...
DB::DB(std::string & path,struct resources *res_ptr_,uint32_t node_id_):thread_pool(4){
    res_ptr = res_ptr_;
    node_id = node_id_;
...
void DB::generate_flush_task(ValueSGE_ptr target_sge_ptr,uint32_t gc_num){
    ...
    flush_th.detach();
    // thread_pool.submit(flush_task,this,target_sge_ptr,gc_num);
...
bool DB::get_from_rocksdb_mem(const std::string &key,uint32_t &sge_id,uint32_t &offset_of_sgement){
    //rocksdb的参数是string *value所以取地址
    std::string value;
    rocksdb::ReadOptions ro;
    ro.only_memtable=true;
    auto status = rocksdb_ptr->Get(ro, key, &value);
    sge_id = *((uint32_t*)value.data());
    offset_of_sgement = *( (uint32_t*)(value.data()+4 ) );
    // if(status.ok()==false){
    //     std::cout<<"get false, key = "<<key<<" sge_id="<<sge_id<<" offset="<<offset_of_sgement<<std::endl;
...
bool DB::get(const std::string &key, std::string &value){
    ...
    if(res==false){
        std::cout<<"get from rocks fail, key = "<<key<< std::endl;
        return false;
    }
    //std::cout<<"get from rocksdb succ,key="<<key<<" value="<<sge_id<<" "<<offset_of_sgement<<std::endl;
    //利用索引信息从value sgement里读
    //先查看内存里的sgement
    get_kv_with_position(sge_id,offset_of_sgement,key,value);
    end = clock();
    duration = (double)(end - beg)/CLOCKS_PER_SEC;
...
void DB::get_kv_with_position(uint32_t sge_id,uint32_t offset_of_sgement,const std::string &key, std::string &value){
...
bool DB::get_memtable(const std::string &key, std::string &value){
    //因为没有自己的memtable所以直接从rocksdb读
    static int get_num=0;
    get_num++;
    uint32_t sge_id;
    uint32_t offset_of_sgement;
    bool res = get_from_rocksdb_mem(key,sge_id,offset_of_sgement);
    if(res==false){
        return false;
    }
    //std::cout<<"get from rocksdb succ,key="<<key<<" value="<<sge_id<<" "<<offset_of_sgement<<std::endl;

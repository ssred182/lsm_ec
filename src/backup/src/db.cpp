...
extern int K;
bool new_lsm = false;
uint32_t max_parity_id=0;
char* recovery_finish_id_list;
std::mutex finish_id_list_lock;
std::vector<int> prioity_id_list;
...
uint32_t max_vsge_id_g=0;
uint32_t pre_order_num=0;
bool DB::get(const std::string &key, std::string &value){
    //因为没有自己的memtable所以直接从rocksdb读
    clock_t beg,end;
    double duration;
    static int get_num=0;
    static double get_lsm_time=0;
    static double get_seg_time=0;
    ...
    if(sge_in_mem){
        ...
        beg = clock();
        NAM_SGE::read_kv_from_sge_for_primary(sge_id,offset_of_sgement,key,value);
        end = clock();
        duration = (double)(end - beg)/CLOCKS_PER_SEC;
        
        if(is_degraded==true){
            get_seg_time += duration;
            if(get_num%10000==0)
                std::cout<<"get_sge_time for degraed_get is "<<get_seg_time<<" ,get_lsm_time is "<<get_lsm_time<<" ,get_num="<<get_num<<std::endl;

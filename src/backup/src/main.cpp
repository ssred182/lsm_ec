            ...
            if(opcode == 1){
               std::cout<<"error!! put operation not allowed in backup node"<<std::endl;
                ...
                if(BACKUP_MODE==1){
                    ...
                    ConReadCache::const_accessor a;
                    if(backup_node.db_list[primary_node_id].con_read_cache.find(a,key) ){
                        status=0;
                        value=a->second;
                    }
                    else{
                ...
                //std::cout<<"evict a set of key for primary_id="<<primary_node_id<<" ,num = "<<key_num<<std::endl;
                int cur_offset=12;
                ...
                for(int i=0;i<key_num;++i){
                    ...
                    if(backup_node.db_list[primary_node_id].con_read_cache.find(a,key)){
                        backup_node.db_list[primary_node_id].con_read_cache.erase(a);
                    }
                    else{
                        //std::cout<<"evict not exist, key="<<key<<std::endl;
                        not_exist_count++;
...
int main(int argc, char *argv[])
{
    if(BACKUP_MODE==2){
        ...
        //backup_node.con_read_cache.reserve(500000);
    }
        ...
        if(poll_result < 0)
        {
            ...
            if(msg_type==1){
                ...
                if(parity_still_valid){
                    static int parity_update_count=0;
                    parity_update_count++;
                    if(parity_update_count%10==0)
                        std::cout<<"parity_update_count="<<parity_update_count<<" backup_parity_num="<<backup_node.parity_sge_num<<" map_size= "<<parity_update_map.size()<<std::endl;
                    std::thread update_parity_th(update_parity_task,poll_id,remove_beg_id,res);
                    update_parity_th.detach();
                }
                else{
                    static int parity_remove_count=0;
                    parity_remove_count++;
                    if(parity_remove_count%10==0)
                        std::cout<<"parity_remove_count="<<parity_remove_count<<std::endl;
                    auto iter=parity_update_map.find(remove_beg_id);
                    if(iter!=parity_update_map.end())
                        parity_update_map.erase(iter);
                ...
                std::cout<<"new push kvs into map,key num= "<<key_num<<" from node:"<<poll_id<<std::endl;
                int cur_offset=0;
                for(int i=0;i<key_num;++i){
                    int key_len;
                    ...
                    // read_cache_lock.unlock();
                    ConReadCache::accessor a;
                    if(backup_node.db_list[poll_id].con_read_cache.find(a,key)==false){
                        backup_node.db_list[poll_id].con_read_cache.insert(a,key);
                    }
                    a->second=std::string(res->msg_buf[poll_id]+cur_offset,value_len);

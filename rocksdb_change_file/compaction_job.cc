...
namespace rocksdb {
std::map<uint32_t,uint32_t> gc_sge_map;
std::mutex gc_sge_map_lock;
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  Slice* last_key=nullptr;
  SequenceNumber last_seq=0;
  Slice* last_value=nullptr;
  ...
  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
    ...
    //here is what i add to record invlid kv pairs of every value segemnt 
    if(last_key!=nullptr&&key.compare(*last_key)==0){
      uint32_t sge_id;
      if(last_seq<c_iter->seq())
        memcpy(&sge_id,last_value->data_,4);
      else
        memcpy(&sge_id,value.data_,4);
      gc_sge_map_lock.lock();
      gc_sge_map[sge_id]++;
      gc_sge_map_lock.unlock();
      static bool is_first=true;
      if(is_first){
        std::cout<<"****** first drop kv in segment with id= "<<sge_id<<std::endl;
        is_first=false;
      }
      static int debug_count=0;
      if(++debug_count%500000==0)
        std::cout<<"compaction has caused "<<debug_count<< " kv pairs invalid and dropped"<<std::endl;
    }
    last_key = (rocksdb::Slice*)&c_iter->key();
    last_seq = c_iter->seq();
    last_value= (rocksdb::Slice*)&c_iter->value();
